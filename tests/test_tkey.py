...
def mock_find_device() -> Generator[MagicMock, None, None]:
    with patch.object(TKey, "_find_device", return_value="/dev/ttyACM0") as mock:
        yield mock
...
def test_tkey_init_and_disconnect(
    mock_get_connection: MagicMock, mock_find_device: MagicMock
) -> None:
    mock_conn = MagicMock()
...
@patch.object(TKey, "_get_connection")
def test_send_and_recv_protocol(mock_get_connection: MagicMock) -> None:
    ...
    assert response[1] == FwRsp.NAME_VERSION.id
    assert response[2:6] == b"tk1 "
    assert response[6:10] == b"mkdf"

    # Verify what was written
    written = bytes(mock_conn.written)
    assert len(written) == 2  # Header (1 byte) + Cmd ID (1 byte)
    # Header: (FID=1 << 5) | (EID=2 << 3) | LenIdx=0 -> 32 | 16 | 0 = 48 (0x30)
    assert written[0] == 0x30
    assert written[1] == FwCmd.NAME_VERSION.id


@patch.object(TKey, "_get_connection")
def test_send_protocol_error_status(mock_get_connection: MagicMock) -> None:
    ...
    tkey = TKey(device=None)
    with pytest.raises(TKeyNOKError) as ctx:
        tkey.send(FwCmd.NAME_VERSION)
    assert "Response status code not OK" in str(ctx.value)


@patch.object(TKey, "_get_connection")
def test_load_app_already_running(mock_get_connection: MagicMock) -> None:
...
def test_load_app_and_sign_flow(mock_get_connection: MagicMock) -> None:
    ...
    assert tkeysign.key_size == 128
    assert tkeysign.sig_size == 64
    ...
    assert pubkey == pubkey_part1 + pubkey_part2
...
@patch.object(TKeySign, "_get_connection")
def test_load_app_hashes_secret(mock_get_connection: MagicMock) -> None:
    ...
    assert load_app_frame[0] == 0x53  # FID=2, EID=2, Status=0, LenIdx=3 -> 0x53
    assert load_app_frame[1] == FwCmd.LOAD_APP.id

    # Verify the payload contents
    payload = load_app_frame[2:]
    # Size (4 bytes, little endian)
    assert payload[0:4] == (13).to_bytes(4, byteorder="little")
    ...
    assert payload[5 : 5 + 32] == expected_hashed_secret
...
def test_sign_app_load_by_digest(
    ...
    assert app0.binary == bin0
    assert app0.version == 3
    assert app0.digest == digest0

    # Test loading by exact digest
    app1 = SignApp.load_mldsa(digest=digest1)
    assert app1.binary == bin1
    assert app1.version == 3
    assert app1.digest == digest1

    # Test loading by prefix
    app2 = SignApp.load_mldsa(digest=digest2[:8])
    assert app2.binary == bin2
    assert app2.version == 4
